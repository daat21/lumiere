...
export function SearchBar({movies,onSearch}:{movies:Movie[],onSearch:(searchedMovies:Movie[])=>void}){
  const [searchQuery,setSearchQuery]=useState('')
  const debouncedSearch=useDebouncedCallback((searchSubString: string)=>{
    const searched = movies.filter(movie=>movie.title.toLowerCase().includes(searchSubString.toLowerCase()))
    onSearch(searched)
